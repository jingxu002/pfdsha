	  ...
	  for iep=1:nep %#ok<ALIGN>
		 ...
		 while srl<=segl %#ok<ALIGN>
			...
			if strcmp(tydis_sc,'principal')==1
				...
				dis_in=min((segl-srl)/10,5);  % distance interval of middle point of surface rupture, unit: km
				...
				pfd(iep,1)=pfd(iep,1)+pmu*pm*prup*pepsrl(iep)*pmd_sr*sum(pdgfd);   % annual probability of exceedance given fault displacement
			else
