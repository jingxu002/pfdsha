function pgfd=pfdsha_5th(sc, gfd)
...
% Calculate only one PFD of one SC of one GFD, then use bsxfun in programs that cll pfdsha to calculate array SC and GFD
% Writter: JingXu  Date: 2017-01-10   Version: 2.0
% load seismicity parameters from 5th generation national seismic 
% hazard map
...
%{
	...
	vfdpe		value of parameters of fault displacement predictive equation, structure variable,
                ...
                name, probability of surface rupture, average displacement, 
                bilinear equation of displacement, bilinear of normalized displacement
    strike      strike, unit: degree, from East, anti-clock wise; 
    lenpp       length, unit: km, length of line segment between nebour points on fault trace
...
load fdpe.mat 
load fault_parameters.mat
seg = fp;
% magnitude bin length, unit: Mw, 
% the interval in practical compute
% in order to void the middle magnitude in compute bin
% as the same of interval frame, so let's bin width to 0.1 
mbl=0.1;
...
if strcmp(tydis_sc,'principal')==1
    ...
    [~,inds_sc_pd]=prin_or_dis(sc_pd,seg.coor);
end
...
% initial annual probability of fault displacement exceedance given 
% fault displacement GFD 
pfd=zeros(nep,1);
...
nm=ceil((mu-m0)/mbl);  % magnitude bin length: mbl Mw
   ...
   for im=1:nm
      ...
      vmi=sf(mi)*(exp(-beta*mag(mi))-exp(-beta*mag(mi+1)))*degr*frac(mi); % probability of earthquake in magnitude interval mi
      pm=(exp(-beta*(m-mbl/2))-exp(-beta*(m+mbl/2)))/(exp(-beta*mag(mi))-exp(-beta*mag(mi+1)))*vmi;   
      % pm, conditional probability of earthquake occur in practical compute bin
      ...
      for iep=1:nep
          ...
          if srl>segl
			...
			if strcmp(tydis_sc,'principal')==1 %#ok<ALIGN>
                % probability of rupture to surface = e^(a+bm)/ [1 + e^(a+bm)]
				temp=vfdpe.rup(1)+vfdpe.rup(2)*m;
				...
				pfd(iep,1)=pfd(iep,1)+vm0*pm*prup*pepsrl(iep)*pmd_sr*sum(pdgfd);   % annual probability of exceedance given fault displacement
			else
